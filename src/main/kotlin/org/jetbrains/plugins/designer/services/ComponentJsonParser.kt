...
object ComponentJsonParser {
    ...
    fun parseAndAddComponents(
        ...
        try {
            ...
            val footerComponentsArray = jsonObject.getAsJsonArray("footerComponents")

            var addedCount = 0

            for (element in componentsArray) {
                addedCount += processComponent(element, screen, componentManager, isFooter = false)
            }

            footerComponentsArray?.forEach { element ->
                addedCount += processComponent(element, screen, componentManager, isFooter = true)
            }
    ...
    private fun processComponent(
        ...
        if (isFooter) {
            componentManager.addComponentToFooter(screen, type, properties)
        } else {
